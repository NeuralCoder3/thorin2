...
std::string Emitter::emit_bb(BB& bb, const Def* def) {
    ...
    if (auto lit = def->isa<Lit>()) {
        ...
        if (isa_lit(index)) {
            ...
            //     return bb.assign(name, "select fast i1 {}, {} {}, {} {} ", ll_idx, elem_t, true_val, elem_t,
            //     false_val);
            // } else {
            //     auto elem_t  = convert(extract->type());
            //     auto index_t = convert(index->type());
            //     print(lam2bb_[entry_].body().emplace_front(),
            //           "{}.alloca = alloca {} ; copy to alloca to emulate extract with store + gep + load", name,
            //           tup_t);
            ...
            //     print(bb.body().emplace_back(), "{}.gep = getelementptr inbounds {}, {}* {}.alloca, i64 0, i64
            //     {}.zext",
